...
public class CircleMechanic : MonoBehaviour
{
	...
	[Range(10, 10)]
	public int ZoneRadiusFactor = 20; //default to 50%
	...
	private Vector3 centerPoint = new Vector3(0, -10, 0);
	...
	private Vector3 NewCenterPoint(Vector3 currentCenter, float currentRadius, float newRadius)
	{
		...
		while (!foundSuitable)
		{
			...
			if (totalCountDown < 1)
				return new Vector3(0, -10, 0);  //explicitly define an error has occured.  In this case we did not locate a reasonable point
		}
		return newPoint;
	}
